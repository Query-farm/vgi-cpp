...
std::shared_ptr<arrow::Schema> hive_schema() {
    return arrow::schema({arrow::field("id", arrow::utf8(), true), bbox(),
                          arrow::field("name", arrow::utf8(), true),
                          arrow::field("num", arrow::int64(), true),
                          arrow::field("theme", arrow::utf8(), true),
...
std::vector<vgi::CatalogTable> rff_tables() {
    ...
    auto rowid = table("rff_rowid",
                       arrow::schema({row_id_field(), bbox(),
